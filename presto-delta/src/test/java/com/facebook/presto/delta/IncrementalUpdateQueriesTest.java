        ...
        extends AbstractDeltaDistributedQueryTestBase
{
    ...
    private final String CONTROL_TABLE_NAME = "deltatbl-partition-prune";
    private final String TARGET_TABLE_NAME = CONTROL_TABLE_NAME + "-incremental";
            ...
            throws Exception
    {
        ...
        VERSION + FileSystems.getDefault().getSeparator() + TARGET_TABLE_NAME,
        VERSION + FileSystems.getDefault().getSeparator() + TARGET_TABLE_NAME);
        return queryRunner;
    }
    ...
    public void readTableNonePartitionedColumnAfterIncrementalUpdateTest()
    {
        String testTableQuery =
                format("SELECT name, cnt FROM \"%s\".\"%s\" order by name, cnt asc", PATH_SCHEMA,
                        goldenTablePathWithPrefix(VERSION, TARGET_TABLE_NAME));
        String controlTableQuery =
                format("SELECT name, cnt FROM \"%s\".\"%s\" order by name, cnt asc", PATH_SCHEMA,
                        goldenTablePathWithPrefix(VERSION, CONTROL_TABLE_NAME));
        checkQueryOutputOnIncrementalWithNullRows(controlTableQuery, testTableQuery);
    }
