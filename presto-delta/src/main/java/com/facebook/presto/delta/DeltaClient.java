...
public class DeltaClient
{
     ...
     */
    public CloseableIterator<FilteredColumnarBatch> listFiles(ConnectorSession session, DeltaConfig deltaConfig, DeltaTableLayoutHandle deltaTableHandle)
    {
        ...
        try {
            ...
            if (enablePushdown) {
                ...
                logger.info("Pushdown enabled: true - Predicate pushdown: " + (predicate.isPresent() ? predicate.get().toString() : " - "));
                scan = predicate.isPresent() ?
                        scanBuilder.withFilter(predicate.get()).build() : scanBuilder.build();
            }
            else {
                logger.info("Pushdown enabled: false");
