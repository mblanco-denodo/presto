 ...
 */
package com.facebook.presto.delta.cache;
 ...
 */
public class DeltaMetadataCache<K, V>
{
    @FunctionalInterface
    ...
    private final Map<K, DeltaMetadataCacheEntry<V>> cache;
     ...
     */
    public DeltaMetadataCache(LoadingFunction<K, V> loadingFunction)
    {
     ...
     */
    public DeltaMetadataCache(LoadingFunction<K, V> loadingFunction, long expirationTimeSeconds)
    {
    ...
    {
        DeltaMetadataCacheEntry<V> entry = cache.get(key);

        if (entry == null) {
            try {
                V value = loadingFunction.apply(key, session, params);
                if (value != null) {
                    entry = new DeltaMetadataCacheEntry<>(value);
                    cache.put(key, entry);
                    return value;
                }
            }
